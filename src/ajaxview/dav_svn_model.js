...
var DavSvnResource = function(name, type, info){
    ...
    this.m_name = name.replace(/%../g, function(s){ return s.toUpperCase(); });
    this.m_children = {};
    this.m_parent = null;
    this.m_info = (info || {});
};
...
var DavSvnModel = function(url, revision, pjax_state){
    url = url.replace(/\/$/, "").replace(/%../g, function(s){ return s.toUpperCase(); });
...
var DavSvnLogModel = function(url, peg_revision){
    this.m_url = url.replace(/\/$/, "").replace(/%../g, function(s){ return s.toUpperCase(); });
